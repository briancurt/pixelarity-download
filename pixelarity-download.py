...
def download_template(template, template_type, force_download, browser):
    ...
    a HTTP 429 message.
    """
    print("[*] Downloading https://pixelarity.com/" + template + "/download/" + template_type + " ...")
    if not path.exists("./px-" + template + "/"):
        makedirs("./px-" + template + "/")
    if not path.exists("./px-" + template + "/px-" + template + "-" + template_type + ".zip") or force_download:
        request = browser.session.get("https://pixelarity.com/" + template + "/download/" + template_type, stream=True)
        if request.status_code == 200:
            with open("./px-" + template + "/px-" + template + "-" + template_type + ".zip", "wb") as temp_zip:
                request.raw.decode_content = True
                copyfileobj(request.raw, temp_zip)
        elif request.status_code == 429:
            print("[*] Server returned HTTP 429 (Too Many Requests), attempting to re-download template " + template + " after 60s timeout")
            sleep(60)
            download_template(template, template_type, force_download, browser)
        else:
            print("[!] Could not download theme " + template)
    else:
        print("[!] Template " + template + "is already downloaded and option --force is not specified, skipping...")
...
def extract_template(template, template_type, force_download):
    ...
    """
    if not path.exists("./px-" + template + "/" + template_type + "/") or force_download:
        makedirs("./px-" + template + "/" + template_type + "/")
        zip_ref = ZipFile("./px-" + template + "/px-" + template + "-" + template_type + ".zip", "r")
...
def main():
        ...
        parser.add_argument('-p', '--psd', help="Download PSD themes too.", action='store_true', required=False)
        parser.add_argument('-x', '--extract', help="Extract templates zipfiles.", action='store_true', required=False)
